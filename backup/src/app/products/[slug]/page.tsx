...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  ...
  const getPricing = (months: number) => {
    if (isLifetime) {
      return { basePrice: product.price, discount: 0, discountedPrice: product.price, savings: 0 }
    }
    const basePrice = product.price * months
    let discount = 0
    if (months === 2) discount = 5
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 lg:gap-4 pb-4 sm:pb-6 border-b border-neutral-200">
                  ...
                  <span className="text-xs sm:text-sm lg:text-base text-neutral-600">{product.reviews} reviews</span>
                  <div className="h-4 sm:h-6 w-px bg-neutral-300 hidden sm:block" />
                  <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                {/* Duration Selection - Only for non-lifetime products */}
                {!isLifetime && (
                  <div>
                    ...
                    {durationOptions.map((option) => {
                      const pricing = getPricing(option.months as 1 | 2 | 3)
                      const isSelected = selectedDuration === option.months
                      return (
                        <button
                          key={option.months}
                          onClick={() => setSelectedDuration(option.months as 1 | 2 | 3)}
                          className={cn(
